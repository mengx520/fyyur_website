...
My part of the project is to build out the data models to power the API endpoints for the Fyyur site by connecting to a PostgreSQL database for storing, querying, and creating information about artists and venues on Fyyur.
...
The frontend is using **HTML**, **CSS**, and **Javascript** with [Bootstrap 3](https://getbootstrap.com/docs/3.4/customize/). 
All frontend dependencies are installed in `static/`, there is no need to install additional files locally.
...
* `app.py` -- (Completed by me.) Defines routes that match the user’s URL, and controllers which handle data and renders views to the user. This is the main file you will be working on to connect to and manipulate the database and render views with data to the user, based on the URL.
* Models in `app.py` -- (Completed by me.) Defines the data models that set up the database tables.
* `config.py` -- (Completed by me.) Stores configuration variables and instructions, separate from the main application code. This is where you will need to connect to the database.
...
3. Connect the application to a real database and talk to a real backend:
...
##### New features to add in the furture
